...
const METRICS = [
  ...
  { id: 'margem_bruta', label: 'Margem Bruta' },
  { id: 'margem_contribuicao', label: 'Margem de Contribuição' },
  { id: 'ebitda', label: 'EBITDA' },
  { id: 'resultado_liquido', label: 'Resultado Líquido' },
];
          ...
          {payload.map((entry: any, index: number) => (
            <div key={index} className="flex flex-col gap-0.5">
              ...
              <span className={`text-lg font-mono font-black tracking-tight ${
                entry.name === "value" ? "text-emerald-400" : "text-slate-400"
              }`}>
...
export const RevenueEvolutionChart = ({ data }: RevenueEvolutionChartProps) => {
  const [activeMetric, setActiveMetric] = useState('receita_bruta');
  ...
  const activeMetricLabel = METRICS.find(m => m.id === activeMetric)?.label || 'Receita Bruta';
  ...
  return (
    <div className="w-full h-[400px] bg-white rounded-xl border border-zinc-100 shadow-[0_20px_50px_-12px_rgba(0,0,0,0.05)] p-5 md:p-6 relative overflow-hidden group flex flex-col">
      {/* Background Decor */}
      <div className="absolute top-0 right-0 w-96 h-96 bg-emerald-500/5 rounded-full blur-3xl -mr-32 -mt-32 transition-all duration-1000 group-hover:bg-emerald-500/10" />
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-4 relative z-10 gap-4">
        <div>
          <h2 className="text-xl md:text-2xl font-black tracking-tight text-slate-900 uppercase">
            Evolução de <span className="text-emerald-600">{activeMetricLabel}</span>
          </h2>
          <p className="text-sm text-zinc-400 font-medium mt-1">
            Performance consolidada dos últimos 12 meses vs. Média Histórica
          </p>
        </div>
          ...
          <ComposedChart data={chartData} margin={{ top: 20, right: 10, left: 0, bottom: 0 }}>
            <defs>
              ...
              <linearGradient id="barGradientDefault" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stopColor="#24445C" stopOpacity={0.95}/>
                <stop offset="95%" stopColor="#24445C" stopOpacity={0.6}/>
              </linearGradient>
